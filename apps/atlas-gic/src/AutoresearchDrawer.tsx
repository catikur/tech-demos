...
import { fmtWeight, type Weights } from "./engine";
...
export function AutoresearchDrawer({
  ...
  const ar = scenario.autoresearch;
  const agent = AGENTS.find((a) => a.id === ar.worstAgentId)!;
  return (
      ...
      <div className="rise-in max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-2xl border border-amber-500/30 bg-zinc-950 p-6 shadow-2xl sm:rounded-2xl">
        <div className="mb-4 flex items-center gap-3">
          ...
          <span className="text-xs text-zinc-500">
            nightly self-improvement · Karpathy-style
          </span>
          <button
            onClick={onClose}
        ...
        <div className="mb-4 rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
          <div className="mb-1 flex items-center gap-2.5">
            ...
            <span className="ml-auto font-mono text-[11px] text-zinc-500">
              weight {fmtWeight(weights[agent.id])}×
            </span>
          </div>
          <p className="font-mono text-[11px] text-zinc-400">{ar.attribution}</p>
        </div>
        ...
        <div className="mb-5 rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
          <p className="mb-2 text-[13px] leading-relaxed text-zinc-300">{ar.rationale}</p>
          <div className="font-mono text-[11px] font-bold text-sky-400">
            replay backtest: {ar.backtestDelta}
          </div>
        </div>
        ...
        {resolution === null ? (
          <div className="flex gap-3">
            ...
            >
              ✓ KEEP — commit tweak, weight +{ar.weightDeltaOnKeep.toFixed(2)}
            </button>
            <button
              onClick={onRevert}
            ...
            >
              ↩ REVERT — discard, keep prior prompt
            </button>
          </div>
        ) : (
          <div
          ...
          >
            {resolution === "keep"
              ? `✓ COMMITTED — ${agent.name} runs the new prompt tomorrow`
              : `↩ REVERTED — checkout restored the prior prompt for ${agent.name}`}
          </div>
        )}

        <p className="mt-4 text-center text-[10px] text-zinc-600">
          Mock cycle: no LLM call is made — the tweak, attribution and backtest numbers are
          seeded demo data.
        </p>
      </div>
    </div>
